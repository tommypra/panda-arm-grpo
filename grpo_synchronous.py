...
class Args:
    ...
    exp_name: str = f"final_grpo_sync_joints_obstacles_mod"
...
if __name__ == "__main__":
    ...
    actions_group = torch.zeros((args.num_steps, args.macro_steps, args.num_envs) + envs.single_action_space.shape).to(device)
    logprobs = torch.zeros((args.num_steps, args.macro_steps)).to(device)
    logprobs_group = torch.zeros((args.num_steps, args.macro_steps, args.num_envs)).to(device)
    rewards = torch.zeros((args.num_steps, args.macro_steps)).to(device)
    rewards_group = torch.zeros((args.num_steps, args.macro_steps, args.num_envs)).to(device)
    dones = torch.zeros((args.num_steps, args.macro_steps)).to(device)
    ...
    for iteration in range(1, args.num_iterations + 1):
        ...
        for step in range(0, args.num_steps):
            # print(f"Step: {step}") if step%100==0 else None
            ...
            with torch.no_grad():
                actions_g, logprobs_g = agent.get_actions(next_obs)
            
            for env_id, env in enumerate(envs.envs):
                if env_id == 0:
                    current_states = env.unwrapped.get_state()
                else:
                    env.unwrapped.set_state(current_states)
            if (iteration % 10 == 0):
            ...
            advantages[step] = (rewards[step] - mean_reward_group) / (std_reward_group + 1e-8)
            ...
            if "final_info" in infos:
                ...
                reward_info = infos['episode']['r'][0]
                length_info = infos['episode']['l'][0]
